...
IMG_SIZE = 384
YOLO_CONF = 0.2
FRAME_SKIP = 2   # use 1 or 2 for tracking stability
...
def build_model(num_classes):
    return timm.create_model("tf_efficientnetv2_l.in21k", pretrained=False, num_classes=num_classes)
...
def create_output_name(ext):
    return f"processed_{datetime.now().strftime('%Y%m%d_%H%M%S')}{ext}"
...
class CorePipeline:
    ...
    def analyze_video(self, input_path):
        ...
        if not os.path.exists(output_path) or os.path.getsize(output_path) == 0:
            raise ValueError("Processed video was not saved correctly.")
